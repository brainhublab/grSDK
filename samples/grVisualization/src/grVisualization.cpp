...
bool GRVisualization::runDataReading()
{
  ...
  /*     std::map<int, GRDevice> availableDevices;
         availableDevices = conn.getAvalibleDevices();
         int l = -1;
         int r = -1;
         for(std::map<int, GRDevice>::const_iterator it = availableDevices.begin(); it != availableDevices.end(); ++it)
         {
  ...
  */
  ...
  for(std::map<int, GRDevice>::const_iterator it = activeDevices.begin(); it != activeDevices.end(); it++)
  {
    d = it->second;
