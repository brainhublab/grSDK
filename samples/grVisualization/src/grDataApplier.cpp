...
bool GRDataApplier::run(std::map<int, GRDevice> &activeDevices)
{
  ...
  int i = 0;
  for(std::vector<GRDevice>::const_iterator it = availableDevices.begin(); it != availableDevices.end(); i++,++it)
  {
    if( it->name == deviceName )
    {
      ...
      activeDevices[deviceId] = *it; // add information about active devices

      conn = devManager.setActiveDevice(deviceId);
      conn->connectSocket();
      //leftArmApplier.run();
...
bool changeFingerData(GRImu* finger)
{
